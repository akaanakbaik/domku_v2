...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!subError && subData) setSubdomains(subData)
      ...
      if (!logError && logData) setActivities(logData)
  ...
  // EFFECT 2: REALTIME LISTENER (Killer Feature)
  useEffect(() => {
    if (!user) return
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'subdomains' }, () => {
        fetchData() // Refresh jika ada subdomain baru (dari API/Web)
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'activity_logs' }, () => {
        fetchData() // Refresh jika ada log baru
      })
      .subscribe()
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      // Data akan auto-refresh karena ada Realtime Listener
  ...
  const handleDelete = async (id) => {
    ...
    // Realtime akan memastikan sinkronisasi ulang
  }
      ...
      <div className="glass rounded-2xl p-6 md:p-8 relative overflow-hidden shadow-2xl transition-all">
        ...
        <div className="flex items-center justify-between mb-6 relative z-10">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-blue-600/20 rounded-xl text-blue-500 shadow-lg shadow-blue-900/20"><Globe size={24} /></div>
            <div><h2 className="text-2xl font-bold text-white">Deploy Subdomain</h2><p className="text-slate-500 text-sm">Create via Web or API.</p></div>
          </div>
          <button onClick={fetchData} className={`p-2 rounded-lg bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-all ${isRefreshing ? 'animate-spin' : ''}`} title="Refresh Data"><RefreshCw size={20}/></button>
        </div>
        ...
        <form onSubmit={handleCreate} className="space-y-6 relative z-10">
          ...
          <button type="submit" disabled={msg.type === 'loading'} className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-blue-600/20 disabled:opacity-50 flex items-center justify-center gap-2 active:scale-[0.99]">{msg.type === 'loading' ? 'Sedang Memproses...' : <><Plus size={20} /> Deploy</>}</button>
        </form>
      </div>

      {/* TABS */}
      <div className="flex gap-6 border-b border-blue-900/30 pb-1">
        <button onClick={() => setActiveTab('domains')} className={`flex items-center gap-2 pb-3 px-2 text-sm font-bold transition-all border-b-2 ${activeTab === 'domains' ? 'text-blue-500 border-blue-500' : 'text-slate-500 border-transparent hover:text-white'}`}><Globe size={16}/> Domains ({subdomains.length})</button>
        <button onClick={() => setActiveTab('history')} className={`flex items-center gap-2 pb-3 px-2 text-sm font-bold transition-all border-b-2 ${activeTab === 'history' ? 'text-blue-500 border-blue-500' : 'text-slate-500 border-transparent hover:text-white'}`}><Activity size={16}/> Logs Activity</button>
      </div>
        ...
        {activeTab === 'domains' && (
          <>
            <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
              ...
              <div className="relative w-full md:w-64"><Search className="absolute left-3 top-2.5 text-slate-500" size={16} /><input type="text" placeholder="Cari domain..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#111318] border border-blue-900/20 rounded-lg py-2 pl-9 pr-4 text-sm text-white focus:border-blue-500 focus:outline-none transition-colors" /></div>
            </div>

            {loading ? <DashboardSkeleton /> : filteredSubdomains.length === 0 ? (
                <div className="text-center py-20 bg-[#111318]/50 rounded-2xl border border-blue-900/20 border-dashed"><Globe size={48} className="mx-auto mb-4 text-blue-900/50"/><p className="text-slate-500 font-medium">Belum ada subdomain.</p></div>
            ) : (
                ...
                {filteredSubdomains.map((item) => (
                  <div key={item.id} className="group glass bg-[#111318]/40 hover:bg-[#161920]/80 rounded-xl p-4 flex flex-col md:flex-row justify-between items-center gap-4 transition-all duration-300 hover:border-blue-500/40 hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-900/10">
                    <div className="flex-1 text-center md:text-left w-full min-w-0">
                      <div className="flex items-center justify-center md:justify-start gap-2 mb-1.5"><h4 className="font-bold text-white text-lg truncate hover:text-blue-400 transition-colors cursor-default">{item.name}</h4><span className={`text-[10px] px-1.5 py-0.5 rounded font-bold border ${item.type === 'A' ? 'border-green-500/30 text-green-400 bg-green-500/10' : 'border-purple-500/30 text-purple-400 bg-purple-500/10'}`}>{item.type}</span></div>
                      <div className="flex items-center justify-center md:justify-start gap-3 text-xs text-slate-500 font-mono"><span className="flex items-center gap-1.5 truncate"><MapPin size={12} className="text-slate-600"/> {item.target}</span><span className="flex items-center gap-1.5 truncate text-slate-600 hidden md:flex">| {item.cf_id === 'unknown' ? 'Web' : 'API/Cloudflare'}</span></div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => setShowQr(item.name)} className="p-2.5 bg-white/5 text-slate-400 hover:text-white rounded-lg border border-white/5 transition-colors hover:bg-blue-600 hover:border-blue-600"><QrCode size={18} /></button>
          ...
          <div className="bg-[#111318]/50 border border-blue-900/20 rounded-xl overflow-hidden">
             <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="bg-black/20 text-slate-400 uppercase text-xs font-bold"><tr><th className="p-4">Aksi</th><th className="p-4">Detail</th><th className="p-4 hidden sm:table-cell">IP</th><th className="p-4 text-right">Waktu</th></tr></thead>
                  <tbody className="divide-y divide-blue-900/10">
                    {activities.length === 0 ? (
                      <tr><td colSpan="4" className="p-8 text-center text-slate-500">Belum ada aktivitas.</td></tr>
                    ) : (
                      activities.map((log) => (
                        <tr key={log.id} className="hover:bg-white/5 transition-colors">
                          <td className="p-4 font-bold text-white flex items-center gap-2">
                            {log.action === 'CREATE_SUBDOMAIN' ? <Globe size={14} className="text-green-500"/> : log.action === 'LOGIN' ? <Check size={14} className="text-blue-500"/> : <Activity size={14} className="text-slate-500"/>}
                            {log.action}
                          </td>
                          <td className="p-4 text-slate-300 max-w-[200px] truncate">{log.details}</td>
                          <td className="p-4 font-mono text-xs text-slate-500 hidden sm:table-cell">{log.ip_address}</td>
                          <td className="p-4 text-right text-slate-500 text-xs"><div className="flex items-center justify-end gap-1"><Clock size={12}/>{new Date(log.created_at).toLocaleString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute:'2-digit' })}</div></td>
                        </tr>
                      ))
                    )}
                  </tbody>
