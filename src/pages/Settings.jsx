...
import { User, Lock, Save, Camera, Mail, Key, ShieldAlert, Loader2, Trash2, AlertTriangle, BadgeCheck, Star, Smartphone, FileText, CheckCircle2, Shield, AlertOctagon, Eye, EyeOff, RefreshCw, Copy } from 'lucide-react'
...
const Settings = () => {
  ...
  // FIX: Fallback user data jika outletContext kosong (penyebab error sebelumnya)
  ...
  const handleProfileUpdate = async (e) => {
    ...
    setLoading(true)
    try {
      // Simulasi update (sesuaikan endpoint API jika ada file upload)
  ...
  const handleRegenerateKey = async () => {
    if (!confirm('API Key lama akan hangus. Lanjutkan?')) return
    setIsRegenerating(true)
    try {
      const res = await fetch('/api/user/apikey', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('domku_token')}` }
      })
      const data = await res.json()
      if (!data.success) throw new Error(data.error)
      addToast('success', 'API Key diperbarui')
      if (refreshSession) refreshSession()
    } catch (err) {
      addToast('error', err.message)
    } finally {
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
            ...
            <div className="text-center md:text-left flex-1 min-w-0 space-y-2">
                <div className="flex items-center justify-center md:justify-start gap-2">
                    ...
                    {userStatus.label === 'ADMINISTRATOR' && <BadgeCheck className="text-blue-500 fill-blue-500/10 shrink-0 animate-pulse" size={18} />}
                </div>
          ...
          <div className="space-y-4">
              ...
              <section className="bg-[#111318] p-5 rounded-2xl border border-white/5 shadow-md relative overflow-hidden group">
                  ...
                  <button 
                    ...
                    className="w-full mt-3 py-2 bg-white/5 hover:bg-white/10 text-slate-300 rounded-xl font-bold text-[10px] transition-all flex items-center justify-center gap-2 border border-white/5"
                  >
                    {isRegenerating ? <Loader2 size={12} className="animate-spin"/> : <RefreshCw size={12}/>} Regenerate Key
                  </button>
