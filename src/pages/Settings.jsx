...
const Settings = () => {
  ...
  const [previewImg, setPreviewImg] = useState(session?.avatar_url || null)
  const [fileToUpload, setFileToUpload] = useState(null)
  
  const [passData, setPassData] = useState({ old: '', new: '' })
  
  // Reset Pass States
  const [resetStep, setResetStep] = useState(0) // 0: none, 1: otp sent
  ...
  const changePassword = async (e) => {
    ...
    try {
      const res = await fetch('/api/user/change-password', {
        ...
        body: JSON.stringify({ email: session.email, oldPassword: passData.old, newPassword: passData.new })
      })
      const data = await res.json()
      if (!data.success) throw new Error(data.error)
  ...
  const confirmReset = async (e) => {
    ...
    try {
      const res = await fetch('/api/user/reset-password-confirm', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: session.email, code: resetOtp, newPassword: resetNewPass })
      })
      const data = await res.json()
      if (!data.success) throw new Error(data.error)
    ...
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="bg-[#111318] border border-blue-900/30 rounded-2xl p-6">
        ...
        <div className="bg-black/40 border border-blue-900/20 p-4 rounded-xl flex items-center justify-between gap-4">
          <code className="text-blue-300 font-mono text-sm break-all">{session?.api_key}</code>
          <button onClick={() => { navigator.clipboard.writeText(session?.api_key); showMsg('success', 'Tersalin!') }} className="p-2 hover:bg-white/10 rounded-lg text-slate-400 hover:text-white transition-colors">
            <Copy size={20} />
          </button>
        </div>
        <p className="text-xs text-slate-500 mt-2">Gunakan key ini untuk mengakses API Domku.</p>
      </div>

      {/* 3. CHANGE PASSWORD */}
      <div className="bg-[#111318] border border-blue-900/30 rounded-2xl p-6">
        <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2"><Lock className="text-green-500"/> Ganti Password</h2>
        ...
        {resetStep === 1 && (
          <div className="mt-6 pt-6 border-t border-blue-900/20 animate-in fade-in slide-in-from-top-2">
            ...
            <form onSubmit={confirmReset} className="space-y-4">
              ...
              <button type="submit" className="w-full py-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold">Reset Password Sekarang</button>
            </form>
          </div>
        )}
      </div>
