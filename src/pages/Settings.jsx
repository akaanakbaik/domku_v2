...
import { User, Lock, Save, Camera, Mail, Key, ShieldAlert, CheckCircle, AlertCircle, Loader2 } from 'lucide-react'
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/user/update-profile', {
        method: 'POST',
        body: formData // Kirim FormData (Header Content-Type otomatis diurus browser)
      })
      ...
      // Update session di localstorage agar sidebar langsung berubah
      ...
      refreshSession() // Trigger refresh global
    ...
    <div className="max-w-4xl mx-auto pb-20 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-5 mb-8">
        <div className="relative group">
            <div className="w-20 h-20 rounded-full bg-blue-600 flex items-center justify-center text-3xl font-bold text-white overflow-hidden border-2 border-blue-400 shadow-lg shadow-blue-900/50">
                {avatarPreview ? (
                  <img src={avatarPreview} alt="Av" className="w-full h-full object-cover"/> 
                ) : (
            ...
            <button 
              ...
              className="absolute bottom-0 right-0 bg-black text-white p-1.5 rounded-full border border-white/20 cursor-pointer hover:bg-blue-600 transition-colors z-10"
              title="Ganti Foto"
            >
                <Camera size={14}/>
            </button>
        ...
        <div>
            ...
            <p className="text-slate-600 text-xs mt-1">Klik ikon kamera untuk ganti foto.</p>
        </div>
      </div>

      {msg.text && (
          <div className={`p-3 rounded-xl flex items-center gap-2 text-sm animate-in zoom-in duration-300 ${msg.type === 'success' ? 'bg-green-900/20 text-green-400 border border-green-500/20' : 'bg-red-900/20 text-red-400 border border-red-500/20'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="glass p-6 rounded-2xl border border-white/5 space-y-6">
              ...
              <form onSubmit={handleProfileUpdate} className="space-y-4">
                  ...
                  <button disabled={loading} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-600/20 disabled:opacity-50">
                      {loading ? <><Loader2 size={18} className="animate-spin"/> Saving...</> : <><Save size={18}/> Simpan Perubahan</>}
                  </button>
              </form>
          </div>
