...
// Konfigurasi Upload (Memory Storage untuk Serverless)
const upload = multer({ 
...
// Limiter Umum (Anti Spam Request)
...
// Limiter Khusus Auth (Anti Brute Force)
const authLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 jam
  max: 20, // Max 20 percobaan login/daftar
  message: { success: false, error: "Terlalu banyak percobaan login. Tunggu 1 jam." }
})

app.use(helmet()) // Secure Headers
app.use(cors({ origin: true, credentials: true })) 
app.use(express.json({ limit: '10kb' })) // Batasi ukuran body json
app.use(limiter) // Terapkan limiter umum

// --- KONEKSI DATABASE & EMAIL ---
...
// REGEX BARU: Huruf, Angka, Titik(.), Strip(-)
...
app.get('/api', (req, res) => {
  res.json({ 
    ...
    version: '4.2.0',
    features: ['Anti-DDoS', 'Rate Limit', 'Cloudflare Check', 'Strict Auth', 'Multi-level Subdomain', 'Extended Profile'] 
  })
})
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
  try {
    ...
    // Cek duplikasi di Auth Supabase
...
app.post('/api/auth/verify-email', authLimiter, async (req, res) => {
  try {
    ...
    // Cek User Hantu (Ghost User) di Auth Supabase
    const { data: { users } } = await supabase.auth.admin.listUsers()
    ...
    if (existing) {
      ...
      const { data: authUser } = await supabase.auth.admin.createUser({
        email: pending.email,
        password: "SECURE_" + crypto.randomBytes(16).toString('hex'), // Password dummy untuk auth internal
    ...
    // Simpan data lengkap ke Public Table
...
// 4. VERIFY OTP (STEP 2 - Login Sukses)
...
// 5. UPDATE PROFILE (Bio & Phone Added)
app.post('/api/user/update-profile', upload.single('avatar'), async (req, res) => {
  try {
    const email = xss(req.body.email)
    const name = xss(req.body.name)
    const bio = xss(req.body.bio || '') // Sanitasi Bio
    const phone = xss(req.body.phone || '') // Sanitasi Phone
    const file = req.file
...
app.post('/api/subdomain', limiter, async (req, res) => {
  ...
  try {
    ...
    // Cek di Cloudflare (Konflik Global)
