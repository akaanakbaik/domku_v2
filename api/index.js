...
app.get('/api', (req, res) => res.json({ status: 'Online', version: '2.2.0' }))
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
  try {
    ...
    if (!name || !email || !password) return res.status(400).json({ success: false, error: "Incomplete data" })

    const { data: userList } = await supabase.auth.admin.listUsers()
    if (userList?.users?.find(u => u.email === email)) return res.status(400).json({ success: false, error: "Email registered" })
...
app.post('/api/auth/verify-email', authLimiter, async (req, res) => {
  try {
    ...
    if (!tokenData) return res.status(400).json({ success: false, error: "Invalid token" })
    if (new Date(tokenData.expires_at) < new Date()) return res.status(400).json({ success: false, error: "Token expired" })

    const email = tokenData.email
    const { data: pending } = await supabase.from('pending_registrations').select('*').eq('email', email).single()
    if (!pending) return res.status(400).json({ success: false, error: "Registration data not found" })
...
app.post('/api/auth/resend-verify', authLimiter, async (req, res) => {
  try {
    ...
    const verifyUrl = `${origin}/verify-email?token=${token}`
    const html = getHtmlTemplate('Link Baru', '<p>Anda meminta link verifikasi baru.</p>', 'Verifikasi', verifyUrl)
    await transporter.sendMail({ from: `"Domku Security" <${process.env.NODEMAILER_EMAIL}>`, to: email, subject: 'Link Verifikasi Baru', html })

    res.json({ success: true, message: 'Resent success' })
  } catch (error) { res.status(500).json({ success: false, error: error.message }) }
})
...
app.post('/api/auth/forgot-password', authLimiter, async (req, res) => {
  try {
    ...
    const html = getHtmlTemplate('Reset Password', `<p>Halo <b>${user.name}</b>,</p><p>Klik tombol untuk reset password.</p>`, 'Ubah Password', resetUrl)
...
app.post('/api/auth/reset-password', authLimiter, async (req, res) => {
  try {
    ...
    if (!tokenData) return res.status(400).json({ success: false, error: "Invalid link" })
...
app.delete('/api/user/delete-account', async (req, res) => {
  try {
    ...
    if(!await bcrypt.compare(password, user.password_hash)) return res.status(403).json({success: false, error: "Password wrong"})
